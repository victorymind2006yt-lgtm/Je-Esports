...
export default function TournamentDetailPage() {
  ...
  const canRegister = currentUser && isUpcoming && !userRegistration && !isFull && !hasStarted;

  let matchStatusMessage = "";
  let matchStatusColor = "bg-amber-500/15 text-amber-200"; // Default

  if (isCancelled) {
    matchStatusMessage = "This tournament has been cancelled.";
    matchStatusColor = "bg-red-500/15 text-red-300";
  } else if (isCompleted) {
    matchStatusMessage = "The tournament has ended.";
    matchStatusColor = "bg-gray-500/15 text-gray-300";
  } else if (hasStarted) {
    matchStatusMessage = "The tournament has started!";
    matchStatusColor = "bg-red-900/20 text-red-400 border border-red-500/20";
  } else {
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,3fr)_minmax(260px,2fr)]">
            ...
            <div className="flex rounded-full bg-white/5 p-1 text-xs sm:text-sm">
              {[
                ...
                <button
                  ...
                  className={`flex-1 rounded-full px-3 py-2 font-medium transition ${activeTab === tab.id
                  ...
                  <div className="rounded-2xl border border-white/10 bg-[#050a0f] px-4 py-4">
                    {isCancelled || hasStarted ? (
                      <div className="py-10 text-center">
                        <Lock
                          className={`mx-auto h-8 w-8 ${isCancelled ? "text-red-400" : "text-white/40"
                            }`}
                        />
          ...
          <aside className="space-y-4">
            <div className="rounded-3xl border border-white/10 bg-[#080f0c] p-5">
              ...
              <div className={`mt-3 rounded-2xl px-3 py-3 text-center text-xs font-semibold ${matchStatusColor}`}>
                {matchStatusMessage}
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-[#080f0c] p-5">
              ...
              <div className="mt-4 space-y-3 text-xs sm:text-sm">
                ...
                {!selectedSlot && !registering && canRegister && (
